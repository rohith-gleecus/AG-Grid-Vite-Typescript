...
export default function getDocDefinition(
  ...
  function getColumnGroupsToExport() {
    ...
    let isColumnGrouping = displayedColumnGroups.some(
      (col: any) => col.hasOwnProperty("children")
    );
  ...
  function createTableCell(cellValue: any, colId: string) {
    const tableCell: any = {
    ...
    if (pdfExportOptions) {
      ...
      if (PDF_WITH_CELL_FORMATTING && styles) {
        Object.entries(styles).forEach(
          ([key, value]: [string, any]) => (tableCell[key] = value)
        );
      }
  ...
  function getPdfExportOptions(colId: string) {
